...
bool endpoint3_handler(cbufptr_t* input_buffer, bufptr_t* output_buffer);
...
template<> struct Codec<encoder_measurements_t> {
    static std::optional<encoder_measurements_t> decode(cbufptr_t* buffer){
        return std::nullopt;
    }
    static bool encode(encoder_measurements_t value, bufptr_t* buffer) {
        void* val0 = &value.encoder_pos_axis0;
        void* val1 = &value.encoder_vel_axis0;
        void* val2 = &value.gpio_axis0;
        void* val3 = &value.encoder_pos_axis1;
        void* val4 = &value.encoder_vel_axis1;
        void* val5 = &value.gpio_axis1;
        return Codec<uint32_t>::encode(*reinterpret_cast<uint32_t*>(val0), buffer)
            && Codec<uint32_t>::encode(*reinterpret_cast<uint32_t*>(val1), buffer)
            && Codec<uint32_t>::encode(*reinterpret_cast<uint32_t*>(val2), buffer)
            && Codec<uint32_t>::encode(*reinterpret_cast<uint32_t*>(val3), buffer)
            && Codec<uint32_t>::encode(*reinterpret_cast<uint32_t*>(val4), buffer)
            && Codec<uint32_t>::encode(*reinterpret_cast<uint32_t*>(val5), buffer);
    }
};
template<> struct Codec<current_command_t> {
